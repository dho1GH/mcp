...
This module is imported and registered ONLY on Huey's agent object in
autogen_workspace.py. No other agent has these functions in its schema,
so no other agent can ever emit a call to them — enforced by what's
wired up, not by instruction.
...
def _headers():
    if not HUE_BRIDGE_IP or not HUE_APP_KEY:
        raise RuntimeError(
            "HUE_BRIDGE_IP and HUE_APP_KEY must be set in .env for Huey to reach the bridge."
        )
